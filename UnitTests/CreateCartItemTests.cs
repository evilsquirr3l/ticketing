...
public class CreateCartItemTests
{
    ...
    [Test]
    public async Task CreateCartItem_MediatrThrowsInvalidOperationException_ReturnsErrorMessage()
    {
        var mediator = new Mock<IMediator>();
        var cartId = Guid.NewGuid();
        var offerId = Guid.NewGuid();
    ...
    [Test]
    public async Task Handle_DatabaseHasValidCartWithAvailableSeat_ReturnsCartItem()
    {
        var cartId = Guid.NewGuid();
        var offerId = Guid.NewGuid();
        var eventId = Guid.NewGuid();
        ...
        Assert.That(result.OfferId, Is.EqualTo(offerId));
    }
